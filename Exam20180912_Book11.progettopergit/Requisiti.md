...
* `FP`  è il numero di risposte fornite dallo studente che non sono corrette per la domanda,
* `FN`  è il numero di risposte corrette per la domanda che non sono state fornite dallo studente.

> Per esempio, se consideriamo una domanda con le risposte `A`, `B`, `C` (correct), and `D` (correct); e una risposta dello studente che include le risposte `B` e `C`, abbiamo:`N=4`, `FP=1` (dato che `B` è nella risposta dello studente ma non è corretta), `FN=1` (dato che `D` è corretta ma non presente nelle risposte dello studente), allora il punteggio sarà `0.5`.
